        ...
        for line in raw_examples:
            ...
            for entity in entities:
                ...
                entity_map[entity["id"]] = { #todo 添加实体id
                    "name": entity_name,
                    "start": entity["start_offset"], #todo 修改start_offset => start
                    "end": entity["end_offset"] #todo 修改end_offset => end
                }
            ...
            for relation in relations:
                predicate = relation["type"] #todo type => label
                subject_id = relation["from_id"] #todo  => subject_text = relation["em1Text"]
                object_id = relation["to_id"] #todo => object_text = relation["em2Text"]
                # The relation prompt is constructed as follows:
                # subject + "的" + predicate
                prompt = entity_map[subject_id]["name"] + "的" + predicate #todo entity_map[subject_id]["name"] => subject_text, 下同
                if entity_map[subject_id]["name"] not in subject_golden:
                    subject_golden.append(entity_map[subject_id]["name"])
                result = {
                    "text": entity_map[object_id]["name"], #todo entity_map[object_id]["name"] => object_text
                    "start": entity_map[object_id]["start"], #todo entity_map[object_id]["start"] => relation["e21start"]
                    "end": entity_map[object_id]["end"] #todo entity_map[object_id]["end"] => relation["e21start"]+object_text.length()
                }
