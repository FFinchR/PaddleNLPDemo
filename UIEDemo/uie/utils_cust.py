        ...
        for line in raw_examples:
            ...
            if "data" in items.keys():
                ...
                if "label" in items.keys():
                    ...
                    text, relations, entities = items["sentText"], items[
                        "relationMentions"], items[
                                                    "entityMentions"]  # todo text => sentText, relations => relationMentions, entities => entityMentions
            texts.append(text)
            ...
            for entity in entities:
                entity_name = text[entity["start"]:entity["end"]]  # todo start_offset => start, end_offset => end, 下同
                entity_map[entity_index] = {  # todo 添加实体id
                    "name": entity_name,
                    "start": entity["start"],
                    "end": entity["end"]
                }
                ...
                result = {
                    "text": entity_name,
                    "start": entity["start"],  # todo start_offset => start
                    "end": entity["end"]  # todo end_offset => end
                }
            ...
            for relation in relations:
                ...
                object_text = relation[
                    "em2Text"]  # todo object_id = relation["to_id"] => object_text = relation["em2Text"]
                # The relation prompt is constructed as follows:
                # subject + "的" + predicate
                prompt = subject_text + "的" + predicate  # todo entity_map[subject_id]["name"] => subject_text, 下同
                if subject_text not in subject_golden:
                    subject_golden.append(subject_text)
                result = {
                    "text": object_text,  # todo entity_map[object_id]["name"] => object_text
                    "start": relation["e21start"],  # todo entity_map[object_id]["start"] => relation["e21start"]
                    "end": relation["e21start"] + len(object_text)
                    # todo entity_map[object_id]["end"] => relation["e21start"]+object_text.length()
                }
